...
All those versiosn can live together and are installed in an isolated directory in user's data folders.
...
# won't work by default as ...
with_pandoc_version("2.11.4", {
  rmarkdown::pandoc_version()
})

# ...rmarkdown = TRUE needs to be  set to use with rmarkdown package functions
with_pandoc_version("2.11.4", rmarkdown = TRUE, {
  rmarkdown::pandoc_version()
})
with_pandoc_version("2.7.3", rmarkdown = TRUE, {
  rmarkdown::pandoc_version()
})
```
